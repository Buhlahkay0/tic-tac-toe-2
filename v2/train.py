...
def self_play_game(net, num_simulations=50):
    ...
    return states, mcts_probs, rewards, current_players
...
def train_network(net, optimizer, states, mcts_probs, rewards, players, epochs=1):
    ...
    for epoch in range(epochs):
        total_loss = 0
        for board, pi, reward, player in zip(states, mcts_probs, rewards, players):
            # Convert board state to tensor using the actual player's perspective.
            board_tensor = board_to_tensor(board, player)
            optimizer.zero_grad()
