...
def board_to_tensor(chess_board, device=device):
    ...
    for square, piece in chess_board.piece_map().items():
        row = 7 - (square // 8)
        col = square % 8
        channel = _WHITE_CHANNELS[piece.piece_type] if piece.color == chess.WHITE \
                  else _BLACK_CHANNELS[piece.piece_type]
...
def boards_to_batch(boards):
    ...
    Keeping the result on CPU lets callers do one .to(device) transfer for the
    whole batch rather than N individual transfers.
    """
    n = len(boards)
    arr = np.zeros((n, 12, 8, 8), dtype=np.float32)
