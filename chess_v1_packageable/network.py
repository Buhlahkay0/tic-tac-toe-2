...
# Set the device to GPU if available (e.g., Nvidia RTX 3070), otherwise fallback to CPU.
...
def board_to_tensor(chess_board, device=device):
    ...
    # Convert to a torch tensor, add a batch dimension, and move to the specified device.
    return torch.from_numpy(board_tensor).unsqueeze(0).to(device)

# Example: create an instance of the network and move it to the GPU.
